...
class Fax < Telefono  
    ...
    def initialize(horario)
        @horarioDesde = horario[:available].to_s.split('-')[0]
        @horarioHasta = horario[:available].to_s.split('-')[1]
        @mensajeContestadora = horario[:custom_message]
        super()
    end

    def llamar(nombre, telefono)  
        tiempoDellamada = Time.now 
        horaLlamada = tiempoDellamada.strftime("%H") 
        puts "ha llamado a las #{horaLlamada}"
        if @horarioDesde <  horaLlamada && @horarioHasta > horaLlamada
            super
        else
            contestadora()
        end
